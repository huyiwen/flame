...
class DensePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(
        ...
        def trunc_norm_kwargs(std):
            return dict(std=std, a=-3 * std, b=3 * std, mean=0.0)
        if isinstance(module, (nn.Linear, nn.Conv1d)):
            # Slightly different from the TF version which uses truncated_normal for initialization
            # cf https://github.com/pytorch/pytorch/pull/5617
            nn.init.trunc_normal_(module.weight, **trunc_norm_kwargs(self.config.initializer_range))
            if module.bias is not None:
                nn.init.zeros_(module.bias)
        elif isinstance(module, nn.Embedding):
            nn.init.trunc_normal_(module.weight, **trunc_norm_kwargs(1e-5))
...
class DenseModel(DensePreTrainedModel):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([DenseBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        if output_attentions:
            warnings.warn(
                "`DenseModel` does not support output attention weights now, so `output_attentions` is set to `False`."
            )
...
class DenseForCausalLM(DensePreTrainedModel, GenerationMixin):
