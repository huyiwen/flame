...
def get_hidden_states_logger(
    ...
    if layer_idx == 0 or layer_idx == num_hidden_layers + 1:
        log_interval = None
    else:
    ...
    def logger(name: str, hidden_states: Optional[torch.Tensor]):
        ...
        if log_interval is None or layer_idx % log_interval == 0:
            hs = {
                f"{prefix}_var/{layer_idx + 1:02}_{name}": torch.var(hidden_states, dim=-1).mean().item(),
                f"{prefix}_mean/{layer_idx + 1:02}_{name}": torch.mean(hidden_states, dim=-1).mean().item(),
                f"{prefix}_max/{layer_idx + 1:02}_{name}": torch.max(hidden_states, dim=-1)[0].mean().item(),
                f"{prefix}_min/{layer_idx + 1:02}_{name}": torch.min(hidden_states, dim=-1)[0].mean().item(),
            }
